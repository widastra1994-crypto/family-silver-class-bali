...
/* ------------------------------------------------------------------ */
/*  SUPABASE — admin auth. Values come from Vite env vars (.env,       */
/*  and the same keys set in Vercel Project Settings -> Environment).  */
/* ------------------------------------------------------------------ */
...
function useAppStateSync(key, value, ready, onStatus) {
  ...
  useEffect(() => {
    ...
    // there's nothing to batch — and a delay only widens the window where a
    // closed tab / quick navigation drops the write before it ever fires.
    const seq = ++seqRef.current;
    if (onStatus) onStatus({ state: "saving", key });
    supabase.from("app_state").upsert({ key, value, updated_at: new Date().toISOString() }).then(({ error }) => {
      // Only a newer save of this same key makes this result stale. A session
      // change must not swallow it, or a save that failed as the session died
      // would never be reported.
      if (seq !== seqRef.current) return;
      if (error) {
        console.error("Gagal menyimpan " + key, error);
        if (onStatus) onStatus({ state: "error", key, message: error.message });
      } else if (onStatus) {
        onStatus({ state: "saved", key });
      }
    });
...
function LoginGate({ onCancel }) {
  ...
  const handleLogin = async () => {
    ...
    if (!supabase) { setError("Supabase belum dikonfigurasi (VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY kosong)."); return; }
    setError("");
    setLoading(true);
  ...
  return (
    <div className="min-h-screen bg-[#16151A] flex items-center justify-center p-5">
      <div className="w-full max-w-sm bg-[#1c1b21] border border-[#2a2930] rounded-2xl p-6">
        <div className="flex items-center gap-2 mb-6">
          <img src={LOGO_SRC} alt="Family Silver Class Bali" className="w-9 h-9 rounded-full object-cover border border-[#3a3940]" />
          <div>
            <p className="text-[#E2E8F0] font-semibold text-sm">Admin Login</p>
            <p className="text-[10px] text-[#6b6a72]">Family Silver Class Bali</p>
          </div>
        </div>
        <div className="space-y-3">
          ...
          <GlowButton className="w-full" onClick={handleLogin}>{loading ? "Memproses..." : "Masuk"}</GlowButton>
          <button onClick={onCancel} className="w-full text-xs text-[#9a99a1] hover:text-[#E2E8F0] text-center py-2">Kembali ke halaman utama</button>
        </div>
      </div>
    </div>
  );
...
function CustomerHeader({ page, setPage, lang, setLang, currency, setCurrency }) {
...
function CustomerPage({ page, setPage, lang, setLang, currency, setCurrency, content, catalog, perGram, extras, reviews, settings, galleryPhotos, guestGalleryPhotos, instructors, asSeenIn, heroPhotos, instagramPhotos, setReservations, blogPosts }) {
  ...
  return (
    <div className="min-h-screen bg-[#16151A]">
      ...
      <CustomerHeader page={page} setPage={setPage} lang={lang} setLang={setLang} currency={currency} setCurrency={setCurrency} />
...
function ContentEditor({ content, setContent, settings, setSettings, extras, setExtras, galleryPhotos, setGalleryPhotos, lang }) {
  ...
  const saveContent = () => {
    // Merge only the fields this editor actually owns (headline/subheadline/promo/
    // description) against the LIVE content[editLang], never replace the whole
    // object with `draft` — draft is a one-time snapshot and would otherwise
    // silently revert directions/packages/about/whyUs/steps/faq/stats to
    // whatever they were when this tab was opened.
    setContent({
      ...content,
      [editLang]: {
        ...content[editLang],
...
function vendorRatePerPax(vendor) {
  ...
  return Number(vendor.ratePerPax) || 0;
}
function parseRatePerPax(value) {
  return value === "" || value === null || value === undefined ? DEFAULT_KOMISI_PER_PAX : Number(value) || 0;
}
...
function AccountingTransactionModal({ open, onClose, onSave, apps, vendors, initial }) {
  ...
  useEffect(() => { setForm(initial ? prepareExisting(initial) : makeBlank()); savingRef.current = false; }, [initial, open]);
  ...
  const addPayment = () => setForm((f) => {
    ...
    return { ...f, instructorPayments: [...f.instructorPayments, { id: "p-" + Date.now(), vendorId, amount: vendorId ? autoAmount(f.pax, vendorId) : "", paid: false, note: "", auto: !!vendorId }] };
  });
  const updatePayment = (id, k, v) => setForm((f) => ({
    ...f,
    instructorPayments: f.instructorPayments.map((p) => {
      if (p.id !== id) return p;
      if (k === "amount") return { ...p, amount: v, auto: false };
      if (k === "vendorId" && p.auto) return { ...p, vendorId: v, amount: autoAmount(f.pax, v) };
      return { ...p, [k]: v };
    }),
  }));
  const recalcPayment = (id) => setForm((f) => ({ ...f, instructorPayments: f.instructorPayments.map((p) => (p.id === id ? { ...p, amount: autoAmount(f.pax, p.vendorId), auto: true } : p)) }));
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      instructorPayments: form.instructorPayments.map(({ auto, ...p }) => ({ ...p, amount: auto ? autoAmount(pax, p.vendorId) : Number(p.amount) || 0 })),
    });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[65] flex items-center justify-center p-4" onClick={onClose}>
      <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-[#1c1b21] border border-[#2a2930] rounded-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 bg-[#1c1b21] border-b border-[#2a2930] p-5 flex items-center justify-between rounded-t-2xl">
          ...
          <div className="border-t border-[#2a2930] pt-5">
            <div className="flex items-center justify-between mb-3 gap-3">
              <div>
                ...
                <p className="text-xs text-[#9a99a1]">Otomatis: jumlah pax × komisi per pax instruktur (atur di Master Data). Ketik angka sendiri kalau berbeda. Centang "Sudah dibayar" setelah transfer.</p>
              </div>
            ...
            <div className="space-y-2 mb-3">
              {form.instructorPayments.map((p) => (
                <div key={p.id} className="bg-[#16151A] border border-[#2a2930] rounded-lg p-3 space-y-2">
                  ...
                  {p.vendorId && (() => {
                    const rate = vendorRatePerPax(vendors.find((v) => v.id === p.vendorId));
                    const formula = (Number(form.pax) || 0) + " pax × " + idr(rate);
                    return p.auto ? (
                      <p className="text-[11px] text-[#C6A15B] px-1">Otomatis: {formula}</p>
                    ) : (
                      <p className="text-[11px] text-[#6b6a72] px-1">
                        Diisi manual ·{" "}
                        <button type="button" onClick={() => recalcPayment(p.id)} className="text-[#C6A15B] hover:underline">Hitung otomatis ({formula})</button>
                      </p>
                    );
                  })()}
                  <div className="flex items-center gap-3">
...
function AccountingOperationalModal({ open, onClose, onSave, initial }) {
  ...
  return (
    <div className="fixed inset-0 z-[65] flex items-center justify-center p-4" onClick={onClose}>
      <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
      <div className="relative w-full max-w-md max-h-[90vh] overflow-y-auto bg-[#1c1b21] border border-[#2a2930] rounded-2xl p-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
...
function AccountingMasterDataTab({ vendors, setVendors, apps, setApps, transactions }) {
  const emptyVendorForm = { name: "", phone: "", note: "", ratePerPax: DEFAULT_KOMISI_PER_PAX };
  const [vendorForm, setVendorForm] = useState(emptyVendorForm);
  const [appForm, setAppForm] = useState("");
  const [editingVendorId, setEditingVendorId] = useState(null);
  const [vendorEditDraft, setVendorEditDraft] = useState(emptyVendorForm);
  ...
  const addVendor = () => {
    ...
    setVendors([...vendors, { id: "vendor-" + Date.now(), ...vendorForm, ratePerPax: parseRatePerPax(vendorForm.ratePerPax) }]);
    setVendorForm(emptyVendorForm);
  };
  const removeVendor = (id) => {
    const linkedCount = transactions.filter((t) => (t.instructorPayments || []).some((p) => p.vendorId === id)).length;
    const msg = linkedCount > 0
      ? "Instruktur ini masih tercatat di " + linkedCount + " rekapan transaksi. Menghapusnya TIDAK menghapus transaksi tersebut, tapi namanya akan tampil sebagai \"(Instruktur dihapus)\" di Laporan Per Instruktur. Tetap hapus?"
      : "Hapus instruktur ini?";
    if (window.confirm(msg)) setVendors(vendors.filter((v) => v.id !== id));
  };
  const startEditVendor = (v) => { setEditingVendorId(v.id); setVendorEditDraft({ name: v.name, phone: v.phone || "", note: v.note || "", ratePerPax: vendorRatePerPax(v) }); };
  const cancelEditVendor = () => setEditingVendorId(null);
  const saveEditVendor = () => {
    if (!vendorEditDraft.name.trim()) return;
    setVendors(vendors.map((v) => (v.id === editingVendorId ? { ...v, ...vendorEditDraft, ratePerPax: parseRatePerPax(vendorEditDraft.ratePerPax) } : v)));
    setEditingVendorId(null);
  };
  const rateInput = (value, onChange) => (
    <div className="relative">
      <input type="number" min="0" step="1000" value={value} onChange={(e) => onChange(e.target.value)} title="Komisi per pax" className={inputCls + " pr-12"} />
      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-[#6b6a72] pointer-events-none">/ pax</span>
    </div>
  );
  ...
  const removeApp = (id) => {
    const linkedCount = transactions.filter((t) => t.appId === id).length;
    const msg = linkedCount > 0
      ? "Aplikasi/Agent ini masih tercatat di " + linkedCount + " rekapan transaksi. Menghapusnya TIDAK menghapus transaksi tersebut, tapi namanya akan tampil sebagai \"(Aplikasi dihapus)\" di Laporan Per Aplikasi/Agent. Tetap hapus?"
      : "Hapus aplikasi ini?";
    if (window.confirm(msg)) setApps(apps.filter((a) => a.id !== id));
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-xs text-[#9a99a1]">Kelola daftar instruktur/pihak yang dibayar dan sumber booking. "Komisi / Pax" dipakai untuk menghitung komisi instruktur otomatis di Rekapan (jumlah pax × komisi per pax).</p>
      </div>
      ...
      <div>
        ...
        <div className="hidden sm:block bg-[#1c1b21] border border-[#2a2930] rounded-xl overflow-hidden mb-4 overflow-x-auto">
          <div className="grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_auto] gap-2 px-4 py-3 bg-[#1f1e24] text-[10px] uppercase tracking-wider text-[#6b6a72] min-w-[680px]">
            <span>Nama</span><span>Telepon</span><span>Komisi / Pax</span><span>Total Komisi</span><span>Belum Dibayar</span><span></span>
          </div>
          {vendors.map((v) => {
            const stat = vendorStats(v.id);
            const isEditing = editingVendorId === v.id;
            return (
              <div key={v.id} className="grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_auto] gap-2 px-4 py-3 border-t border-[#2a2930] items-center text-sm min-w-[680px]">
                {isEditing ? (
                  <>
                    ...
                    {rateInput(vendorEditDraft.ratePerPax, (val) => setVendorEditDraft({ ...vendorEditDraft, ratePerPax: val }))}
                  ...
                  <>
                    ...
                    <span className="text-[#C6A15B]">{idr(vendorRatePerPax(v))}</span>
                    <span className="text-[#E2E8F0]">{idr(stat.total)}</span>
                    <span className={stat.unpaid > 0 ? "text-yellow-400 font-semibold" : "text-[#6b6a72]"}>{stat.unpaid > 0 ? idr(stat.unpaid) : "Lunas"}</span>
                    <div className="flex items-center gap-2 justify-self-end">
              ...
              <div key={v.id} className="bg-[#1c1b21] border border-[#2a2930] rounded-xl p-4">
                {isEditing ? (
                  <div className="space-y-2">
                    ...
                    <input value={vendorEditDraft.phone} onChange={(e) => setVendorEditDraft({ ...vendorEditDraft, phone: e.target.value })} placeholder="Telepon" className={inputCls} />
                    <input value={vendorEditDraft.note} onChange={(e) => setVendorEditDraft({ ...vendorEditDraft, note: e.target.value })} placeholder="Catatan" className={inputCls} />
                    <label className="text-xs text-[#9a99a1] block">Komisi per pax</label>
                    {rateInput(vendorEditDraft.ratePerPax, (val) => setVendorEditDraft({ ...vendorEditDraft, ratePerPax: val }))}
                    <div className="flex gap-2">
                  ...
                  <>
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div>
                        ...
                        <p className="text-xs text-[#C6A15B] mt-0.5">Komisi per pax: {idr(vendorRatePerPax(v))}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <button onClick={() => startEditVendor(v)} className="text-[#9a99a1] hover:text-[#C6A15B]"><Edit3 className="w-4 h-4" /></button>
                        <button onClick={() => removeVendor(v.id)} className="text-[#6b6a72] hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
        ...
        <div className="bg-[#1c1b21] border border-[#C6A15B]/20 rounded-xl p-4 grid sm:grid-cols-[1.5fr_1fr_1fr_1fr_auto] gap-2">
          ...
          {rateInput(vendorForm.ratePerPax, (val) => setVendorForm({ ...vendorForm, ratePerPax: val }))}
...
function AccountingLaporanTab({ transactions, operational, vendors, apps }) {
  ...
  // commission never silently disappears from these reports while still
  // counting toward the Rekapan Utama / Laba Rugi totals above.
  const appBreakdown = [];
  buildPeriods(viewMode).forEach((p) => {
    const appNameById = new Map(apps.map((a) => [a.id, a.name]));
    const idsInPeriod = new Set();
    transactions.forEach((t) => { if (t.appId && p.test(new Date(t.date))) idsInPeriod.add(t.appId); });
    idsInPeriod.forEach((appId) => {
      const txs = transactions.filter((t) => t.appId === appId && p.test(new Date(t.date)));
      if (txs.length > 0) appBreakdown.push({ period: p.key, name: appNameById.get(appId) || "(Aplikasi dihapus)", appId, periodTest: p.test, count: txs.length, revenue: txs.reduce((s, t) => s + txRevenue(t), 0) });
    });
  });
  ...
  buildPeriods(viewMode).forEach((p) => {
    ...
    transactions.forEach((t) => {
      ...
      (t.instructorPayments || []).forEach((pay) => { if (pay.vendorId) idsInPeriod.add(pay.vendorId); });
    });
    idsInPeriod.forEach((vendorId) => {
      let total = 0, unpaid = 0, count = 0;
      transactions.forEach((t) => {
        if (!p.test(new Date(t.date))) return;
        (t.instructorPayments || []).forEach((pay) => {
          if (pay.vendorId === vendorId) { total += Number(pay.amount) || 0; count += 1; if (!pay.paid) unpaid += Number(pay.amount) || 0; }
        });
      });
      if (count > 0) vendorBreakdown.push({ period: p.key, name: vendorNameById.get(vendorId) || "(Instruktur dihapus)", vendorId, periodTest: p.test, count, total, unpaid });
    });
  });
...
function SaveStatusBadge({ status }) {
  ...
  if (status.state === "saving") {
    return (
      <span className="flex items-center gap-1.5 text-xs text-[#9a99a1]">
        <span className="w-1.5 h-1.5 rounded-full bg-[#C6A15B] animate-pulse" /> Menyimpan...
      </span>
    );
  }
  if (status.state === "error") {
    return (
      <span className="flex items-center gap-1.5 text-xs text-red-400" title={status.message || ""}>
        <AlertTriangle className="w-3.5 h-3.5" /> Gagal menyimpan — periksa koneksi Anda
      </span>
    );
  }
  return (
    <span className="flex items-center gap-1.5 text-xs text-green-400">
      <CheckCircle2 className="w-3.5 h-3.5" /> Tersimpan ke server
    </span>
  );
}
      ...
      <header className="sticky top-0 z-40 backdrop-blur-md bg-[#16151A]/85 border-b border-[#2a2930] min-h-16 flex flex-wrap items-center justify-between gap-y-2 px-5 sm:px-8 py-2">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            title="Logout"
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border border-[#3a3940] text-[#9a99a1] hover:text-red-400 hover:border-red-400/40 transition-colors"
          >
...
export default function FamilySilverClassBaliApp() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setSaveStatus((s) => (s.state === "saved" ? { state: "idle" } : s)), 2000);
  ...
  useEffect(() => {
    ...
    setReloginMsg("");
    // Keep a pending error visible so the admin learns the last save failed.
    setSaveStatus((s) => (s.state === "error" ? s : { state: "idle" }));
    async function init() {
      ...
      if (!map) { setStateLoadError(true); return; }
      ...
      // The map is still applied above so the public site shows real content,
      // but the admin stays locked until reservations load too.
      if (userId && !rsv) { setStateLoadError(true); return; }
  ...
  useAppStateSync("perGram", perGram, stateReady, setSaveStatus);
  useAppStateSync("extras", extras, stateReady, setSaveStatus);
  useAppStateSync("settings", settings, stateReady, setSaveStatus);
  useAppStateSync("galleryPhotos", galleryPhotos, stateReady, setSaveStatus);
  useAppStateSync("reviews", reviews, stateReady, setSaveStatus);
  useAppStateSync("heroPhotos", heroPhotos, stateReady, setSaveStatus);
  useAppStateSync("guestGalleryPhotos", guestGalleryPhotos, stateReady, setSaveStatus);
  useAppStateSync("instructors", instructors, stateReady, setSaveStatus);
  useAppStateSync("asSeenIn", asSeenIn, stateReady, setSaveStatus);
  useAppStateSync("instagramPhotos", instagramPhotos, stateReady, setSaveStatus);
  useAppStateSync("accountingTransactions", accountingTransactions, stateReady, setSaveStatus);
  useAppStateSync("accountingOperational", accountingOperational, stateReady, setSaveStatus);
  useAppStateSync("accountingVendors", accountingVendors, stateReady, setSaveStatus);
  useAppStateSync("accountingApps", accountingApps, stateReady, setSaveStatus);
  useAppStateSync("blogPosts", blogPosts, stateReady, setSaveStatus);
        ...
        <div className="min-h-screen bg-[#16151A] flex items-center justify-center text-[#6b6a72] text-sm">
          {stateLoadError ? (
            <div className="text-center px-6">
              ...
              <p className="mb-4">Periksa koneksi internet Anda, lalu coba lagi. Setelah koneksi kembali, mungkin perlu menunggu sekitar 1 menit.</p>
              <div className="flex flex-wrap items-center justify-center gap-3">
                ...
                <button
                  onClick={async () => {
                    ...
                    if (error) setReloginMsg("Server login belum bisa dihubungi. Tunggu sekitar 1 menit, lalu coba lagi.");
                  }}
                  className="px-5 py-2 rounded-full border border-[#2a2930] text-[#E2E8F0] hover:border-[#C6A15B]"
                >
                  Login ulang
                </button>
                <button onClick={() => setMode("customer")} className="px-5 py-2 rounded-full text-[#9a99a1] hover:text-[#E2E8F0]">Kembali ke website</button>
              </div>
              {reloginMsg && <p className="mt-4 text-[#e0ac6a]">{reloginMsg}</p>}
            </div>
